...
class Package:
    ...
    def __str__(self): # return some def str, we can use the 'self' reference to perhaps refer to some of our initialized obj attributes to return 
        return f"Package: {self.id}. Sender: {self.sender}. Recipient: {self.recipient}. Weight: {self.weight}"
...
def main():
    ...
    """
    # so now, how to print our objs, perhaps with a __str__(self) special method inside of our class --> Yes?
    for i in packages:
        print(i) # now, since we've defined special method __str__(self), now we can print our obj(s) simply by calling them (in this case with the iterator variables inside of our list 'packages' with our obj(s) as elements)
